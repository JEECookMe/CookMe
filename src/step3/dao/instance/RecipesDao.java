...
public class RecipesDao {
	...
	public ArrayList<RecipeModelBean> getAllRecipes() {
		...
		try {
        	...
        	while(rs.next()){
        		recipeList.add(new RecipeModelBean(rs.getString("title"), rs.getString("description"), rs.getInt("expertise"), rs.getInt("nbpeople"), rs.getInt("duration"), rs.getString("type")));
        	}
        	rs.close();
