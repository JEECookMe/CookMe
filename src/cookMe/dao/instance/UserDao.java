...
public class UserDao {
    ...
    public void addUser(UserModelBean user) {
        ...
        try {
            ...
            query = connection.prepareStatement("insert into table_user(firstname, lastname, age, email, login, pwd, admin) values (?,?,?,?,?,?,?)");
    ...
    public ArrayList<UserModelBean> getAllUser() {
        ...
        try {
            ...
            java.sql.ResultSet rs = query.executeQuery("select firstname, lastname, age, email, login, pwd, admin from table_user");
            while (rs.next()) {
                userList.add(new UserModelBean(rs.getString("firstname"), rs.getString("lastname"), rs.getInt("age"), rs.getString("email"), rs.getString("login"), rs.getString("pwd"), rs.getBoolean("admin")));
            }
            rs.close();
        ...
        try {
            ...
            if (rs.next()) {
                userBean = new UserModelBean(rs.getString("firstname"), rs.getString("lastname"), rs.getInt("age"),
                        rs.getString("email"), rs.getString("login"), rs.getString("pwd"), rs.getBoolean("admin"));
            }
    ...
    public UserModelBean getUserByLogin(String login) {
        UserModelBean userBean = null;
    ...
    public boolean deleteUser(String login) {
        ...
        try {
            ...
            PreparedStatement query = connection.prepareStatement("DELETE  from table_user where login = ?");
            query.setString(1, login);
            query.execute();
            result = true;
            connection.close();
    ...
    public boolean updateUser(UserModelBean user) {
        ...
        try {
            ...
            PreparedStatement query = connection.prepareStatement("UPDATE table_user  SET  firstname= ?, lastname= ?,age= ?, email=?, login=?, pwd=?, admin=?  WHERE login= ?");
            query.setString(1, user.getFirstName());
            query.setString(2, user.getLastName());
            query.setInt(3, user.getAge());
            query.setString(4, user.getEmail());
